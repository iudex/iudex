...
contract EthBtcEscrow is usingOraclize, usingIudex {
  ...
  function escrow(uint _mBTC, string _btcAddr, address _ethAddr,
                    uint _timestampLimit) coupon("HackEtherCamp") {
    mBTC = _mBTC;
    ethAddr = _ethAddr;
  ...
  function __callback(bytes32 myid, string result) coupon("HackEtherCamp") {
